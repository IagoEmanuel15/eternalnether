package com.izofar.bygonenether.world.entity.ai.sensing;
...
public class ModPiglinBruteSpecificSensor extends PiglinBruteSpecificSensor {
    ...
    public Set<MemoryModuleType<?>> requires() {
        return ImmutableSet.<MemoryModuleType<?>>builder()
                ...
                .add(ModMemoryModuleTypes.NEAREST_TARGETABLE_PLAYER_NOT_WEARING_GILD_MEMORY_MODULE_TYPE.value())
                .build();
    }
    ...
    protected void doTick(ServerLevel level, LivingEntity entity) {
        ...
        for (LivingEntity livingEntity : nearestVisibleLivingEntities.findAll(Predicates.alwaysTrue())) {
            if (livingEntity instanceof Player player && optional.isEmpty() &&
                    !ModPiglinBruteAi.isWearingGild(player) && entity.canAttack(livingEntity)) {
                optional = Optional.of(player);
            }
        }
        brain.setMemory(ModMemoryModuleTypes.NEAREST_TARGETABLE_PLAYER_NOT_WEARING_GILD_MEMORY_MODULE_TYPE.value(),
                optional);
    }
}
